...
export async function createPayment(
  ...
  const payment = await prisma.$transaction(async (tx) => {
    ...
    const paidCentsAfter = getConfirmedPaidCents(createdPayment.invoice.payments);
    const nextInvoiceStatus = paidCentsAfter >= createdPayment.invoice.totalCents ? 'paid' : 'issued';
